...
export async function POST(request: Request) {
  const debug = (...args: any[]) => console.log('[SMART-NOTE-INLINE-IDEAS]', ...args);
  const startTime = Date.now();
  const requestId = Math.random().toString(36).substring(7);

  console.log(`[${requestId}] Smart note inline idea suggestions request started`, {
  ...
  try {
    ...
    debug('Parsed request data:', { noteId, noteContent: noteContent?.length, limit });
    ...
    debug('Backend URL:', `${BACKEND_URL}/api/v1/smart-notes-inline/note-idea-suggestions`);
