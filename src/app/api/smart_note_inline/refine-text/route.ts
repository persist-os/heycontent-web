...
export async function POST(request: Request) {
  const debug = (...args: any[]) => console.log('[SMART-NOTE-INLINE-REFINE-TEXT]', ...args);
  const startTime = Date.now();
  const requestId = Math.random().toString(36).substring(7);

  console.log(`[${requestId}] Smart note inline text refinement request started`, {
  ...
  try {
    ...
    debug('Parsed request data:', { 
      refinement_type, 
      selected_text: selected_text?.length, 
      note_type, 
      surrounding_context: surrounding_context ? {
        before_text: surrounding_context.before_text?.length,
        after_text: surrounding_context.after_text?.length,
        selection_position: surrounding_context.selection_position,
        note_title: surrounding_context.note_title
      } : null,
      refinement_intensity 
    });
    ...
    // Prepare payload for backend (already in correct format)
    const payload = {
      refinement_type,
      selected_text,
    ...
    debug('Prepared payload for backend:', { 
      ...payload, 
      selected_text: payload.selected_text?.length,
      surrounding_context: {
        ...payload.surrounding_context,
        before_text: payload.surrounding_context.before_text?.length,
        after_text: payload.surrounding_context.after_text?.length
      }
    });
    ...
    debug('Backend URL:', `${BACKEND_URL}/api/v1/smart-notes/refine-text`);
    ...
    if (!response.ok) {
      ...
      if (backendData && backendData.detail) {
        ...
        if (typeof backendData.detail === 'object' && backendData.detail.message) {
          // 🎯 FIX: Handle validation failures more gracefully
          if (backendData.detail.error_type === 'validation_failure' && backendData.detail.recoverable) {
            ...
            debug('Backend validation failed but recoverable - returning original text as fallback');
          ...
          if (errorMessage.includes('Unsupported refinement type')) {
            debug('Backend returned unsupported refinement type - this should now be handled with fallbacks');
            errorMessage = 'This refinement type is not yet supported. Please try a different option.';
          }
          
          // 🎯 FIX: Handle text length validation failures
          if (errorMessage.includes('extremely short') || errorMessage.includes('absolute min')) {
            debug('Backend text length validation failed - returning original text as fallback');
    ...
    // Extract the actual refinement response from the wrapper
    ...
    // Return the refined response data (unwrapped if it was wrapped)
