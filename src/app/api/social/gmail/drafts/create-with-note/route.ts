...
export async function POST(req: NextRequest) {
  try {
    const { to, subject, body, thread_id, partnership_context } = await req.json();
    if (!to || !subject || !body) {
      return NextResponse.json({ status: 'error', error: 'Missing required fields: to, subject, or body' }, { status: 400 });
    }
    ...
    const response = await fetch(`${backendUrl}/gmail/drafts/create-with-note`, {
      method: 'POST',
      headers: {
