        ...
        try {
          ...
          for (const mention of mentions) {
            if (mention.type === 'content') {
              // Handle content mentions (email, video, etc.)
              try {
                if (mention.subtype === 'email' || mention.subtype === 'email_thread') {
                  const emailData = await new Promise((resolve) => {
                    resolve(null); // Placeholder - would need actual API call here
                  });
                  
                  if (emailData) {
                    mentionContexts.push(`Email Reference - ${mention.title}: [Email data would be included here]`);
                  }
                  ...
                  if (videoData) {
                    mentionContexts.push(`Video Reference - ${mention.title}: [Video data would be included here]`);
                  }
                }
              } catch (error) {
                console.warn('Failed to fetch content data for mention:', mention.id, error);
              }
              ...
              try {
                ...
                if (platformData) {
                  mentionContexts.push(`Platform Reference - ${mention.title}: [Platform data would be included here]`);
                }
              } catch (error) {
                console.warn('Failed to fetch platform data for mention:', mention.id, error);
              }
            }
          }
