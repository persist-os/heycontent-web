...
export const InsightCard: React.FC<InsightCardProps> = ({
  ...
  const discussFullInsight = () => {
    ...
    const fullInsightContext = {
      ...
      contentId: `content-hub-ai-insight-${Date.now()}`,
      title: title,
      source: 'content-hub',
      ...
      content: `## ${title}\n\n**Platform:** ${validatedPlatform.toUpperCase()}\n**Impact:** ${cleanImpact}\n\n### Why Now?\n${whyNow.map(reason => `• ${reason}`).join('\n')}\n\n### Action Steps\n${actionSteps.map((step, idx) => `${idx + 1}. ${step}`).join('\n')}\n\n### Expected Outcome\n${expectedOutcome}\n\n### Source Details\n${sourceDetails.join('\n')}\n\n${relatedItems && relatedItems.length > 0 ? `### Related Items\n${relatedItems.map(item => `• ${item.label}: ${item.value}`).join('\n')}` : ''}\n\n**Source:** Content Hub AI Insights\n**Type:** Platform-Specific Insight\n\nThis insight was generated by analyzing your ${validatedPlatform} content to identify strategic opportunities for growth and optimization.`,
      insight: `**Platform:** ${validatedPlatform.toUpperCase()}\n**Impact:** ${cleanImpact}\n\n### Why Now?\n${whyNow.map(reason => `• ${reason}`).join('\n')}\n\n### Action Steps\n${actionSteps.map((step, idx) => `${idx + 1}. ${step}`).join('\n')}\n\n### Expected Outcome\n${expectedOutcome}\n\n### Source Details\n${sourceDetails.join('\n')}\n\n${relatedItems && relatedItems.length > 0 ? `### Related Items\n${relatedItems.map(item => `• ${item.label}: ${item.value}`).join('\n')}` : ''}`,
      analysis: `**${title}**\n\n**Platform:** ${validatedPlatform}\n**Source:** Content Hub AI Insights\n\n**Insight:**\n**Platform:** ${validatedPlatform.toUpperCase()}\n**Impact:** ${cleanImpact}\n\n### Why Now?\n${whyNow.map(reason => `• ${reason}`).join('\n')}\n\n### Action Steps\n${actionSteps.map((step, idx) => `${idx + 1}. ${step}`).join('\n')}\n\n### Expected Outcome\n${expectedOutcome}\n\n### Source Details\n${sourceDetails.join('\n')}\n\n${relatedItems && relatedItems.length > 0 ? `### Related Items\n${relatedItems.map(item => `• ${item.label}: ${item.value}`).join('\n')}` : ''}\n\nThis ${validatedPlatform} insight was generated by analyzing your content to identify strategic opportunities for growth and optimization.`,
      timestamp: Date.now(),
      type: 'content-hub-ai-insight'
    };
    console.log('[DISCUSS WITH CONTENT] Setting context:', fullInsightContext);
    setAIInsightsContext(fullInsightContext);
    console.log('[DISCUSS WITH CONTENT] Context set in store. Navigating to chat...');
    setTimeout(() => {
      console.log('[DISCUSS WITH CONTENT] Navigating to /dashboard/chat');
            ...
            <div className="space-y-2">
              {actionSteps.map((step, idx) => (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    console.log('[ACTION STEP] Clicked:', { step, idx, title, platform: validatedPlatform });
                    ...
                    const context = {
                      ...
                      contentType: 'action-step',
                      additionalContext
                    };
                    console.log('[ACTION STEP] Built context:', context);
                    setAIInsightsContext(context);
                    console.log('[ACTION STEP] Context set in store. Navigating to chat...');
                    setTimeout(() => {
                      console.log('[ACTION STEP] Navigating to /dashboard/chat?autoSend=true');
                      router.push('/dashboard/chat?autoSend=true');
                    }, 100);
                  }}
                >
