...
export async function POST(request: Request) {
  try {
    ...
    const convexUser = await convex.query(api.queries.getUserById, { userId: decodedToken.uid });
    ...
    if (!convexUser) {
      ...
      console.log('User found in Convex:', convexUser);
    }
    ...
    // Set the Firebase auth token cookie if it doesn't exist
    if (!existingCookie) {
