...
/**
 * Get latest A2A notes with optional filtering (internal - for HTTP endpoints)
 */
...
/**
 * Get latest A2A notes with optional filtering (public - for frontend)
 * 
 * WHY: A2A notes are stored in Convex, so frontend can query directly.
 * No need for HTTP proxy - Convex reactive queries provide real-time updates.
 */
export const getLatestA2ANotesPublic = query({
  args: {
    conversationId: v.optional(v.string()),
    projectId: v.optional(v.string()),
    limit: v.optional(v.number()),
