...
/**
 * Claude Import Tracking
 * 
 * Tracks one-time Claude conversation imports to prevent duplicate imports.
 * Users can only import their Claude data once.
 * 
 * NOTE: This file demonstrates the extensibility of the data_imports table.
 * Implementation follows the same pattern as chatgptImport.ts
 */

const IMPORT_SOURCE = "claude" as const;

/**
 * Check if user has already imported Claude data
 */
...
/**
 * Mark Claude import as completed
 */
...
/**
 * Cancel Claude import and delete all imported data
 */
export const cancelImport = mutation({
  ...
  handler: async (ctx, { userId, reason }) => {
    console.log(`[CLAUDE_IMPORT] Canceling import and deleting all data for user ${userId}`);
    ...
    const relatedJobs = await ctx.db
      ...
      .filter((q) => 
        q.or(
          q.eq(q.field("type"), "claude_import"),
    ...
    const shardsToDelete = await ctx.db
      ...
      .withIndex("by_user", (q) => q.eq("userId", userId))
      .filter(q => q.eq(q.field("source"), "claude_import"))
      .collect();
    ...
    const crystalsToDelete = allUserCrystals.filter(crystal => 
      crystal.tags?.includes("claude_import")
    );
...
/**
 * Get all background jobs related to Claude import
 */
